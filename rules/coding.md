...
Before introducing any new type or struct or helper method, please deeply search within codebase for some type or method that can be reused or extended easily to achieve goal.
Please try to achieve goal with the least amount of code possible.
Don't introduce helper methods that does very simple thing and it is used only in one place.
When introducing new functions into codebase, first introduce the function itself, then function call in existing codebase.
Don't make changes with only change that is empty line.
For statuses prefer enums.
Write plans compact, I don't want to read very long obvious things in plan. Compact!
